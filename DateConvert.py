...
class Lunar(object):
    ...
    # 时辰
    sc = '早子时丑时寅时卯时辰时巳时午时未时申时酉时戌时亥时晚子时'
    # 节气
...
def ba_zi(ct):
    ln = Lunar(ct)
    print('公历 {}  时间 {}'.format(ln.localtime.date(), ln.localtime.time()))
    print('{} {}时    {}年 {}月 {}日 {}时    {}年'.format(ln.ln_date_str(), ln.gz_hour(), ln.gz_year(), ln.gz_month(), ln.gz_day(), ln.gz_hour(),  ln.sx_year()))
#   print('{} {}年'.format(ln.ln_date_str(), ln.sx_year()))
#   print('年本命 {}'.format(ln.nian_ben_ming[ln.gz_year()]))

    print('八字: {} {} {} {}'.format(ln.gz_year(), ln.gz_month(), ln.gz_day(), ln.gz_hour()))
    print('五行: {} {} {} {}'.format(ln.gz_to_wu_xing(ln.gz_year()), ln.gz_to_wu_xing(ln.gz_month()), ln.gz_to_wu_xing(ln.gz_day()), ln.gz_to_wu_xing(ln.gz_hour())))
...
sys.stdout = Logger('test.xls', sys.stdout)
...
#   ct0 = datetime.datetime(2019, 7, 16, 10, 50, 15)
    ct =datetime.datetime(1988, 12, 26, 23, 6, 15)
